...
class Maze:
    ...
    def __init__(self, rows: int = 10, cols: int = 10,
        ...
        '''
        if not isinstance(prop_blocked, float) or prop_blocked < 0 or prop_blocked > 1:
            raise ValueError("prop_blocked must be a float between 0 and 1")
    ...
    def getNumCellsExplored(self) -> int:
        return self._num_cells_explored

    def getPathLength(self) -> int:
        return self._path_length
    
    def manhattanDistance(self, cell1: Cell, cell2: Cell) -> int:
    ...
    def aStar(self) -> Cell | None:
        ...
        start = self._start
        g_start = 0.0
        h_start = self.manhattanDistance(start, self._goal)
        f_start = g_start + h_start

        # Track parent and costs in Cell objects
        start.setParent(None)

        # Add to open set
        to_explore.insert(f_start, start)
        explored[start.getPosition()] = g_start

        self._num_cells_explored = 1

        while not to_explore.isEmpty():
            # Get node with lowest f-score
            current_entry = to_explore.removeMin()
            current = current_entry.value
            ...
            for neighbor in self.getSearchLocations(current):
                ...
                tentative_g = explored[current.getPosition()] + 1

                # Check if this path to neighbor is better
                if (neighbor.getPosition() not in explored or
                    tentative_g < explored[neighbor.getPosition()]):

                    # Update neighbor's information
                    explored[neighbor.getPosition()] = tentative_g
                    h = self.manhattanDistance(neighbor, self._goal)
                    f = tentative_g + h

                    # Add to open set
                    to_explore.insert(f, neighbor)
                    neighbor.setParent(current)
                    self._num_cells_explored += 1
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        if goal_bfs:
            m.showPath(goal_bfs)
            print(f"Path length: {m.getPathLength()}")
            print(f"Cells explored: {m.getNumCellsExplored()}")
        else:
